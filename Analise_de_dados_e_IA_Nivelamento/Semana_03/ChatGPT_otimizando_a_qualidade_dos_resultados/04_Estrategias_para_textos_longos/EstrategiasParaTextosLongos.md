...
A dica principal para texto longos de mais e quebrar o texto, ou seja diminuir o input em menores, um ponto de atenção e verificar a quantidade de caracteres possíveis pelo chat.
Ou seja temos 2 pontos a serem visualizados:
1) QUbre em partes menores
2) Trabalhar em paralelo  

Uma coisa que pode ser utilizada de forma programática é utilizando a [Langchain](https://github.com/langchain-ai/langchain)
Essa é uma biblioteca `Python` que é utilizada, para verificar a quantidade de tokens sobre um prompt em questão.  

## 2. Tentando resumir um texto com muitos caracteres
Carla precisa fazer um trabalho de um MBA que está cursando, sobre um artigo científico longo e complexo. Ela decidiu usar o ChatGPT para ajudá-la a resumir o texto e facilitar a sua compreensão. No entanto, ela não sabe como usar o ChatGPT para resumir textos longos e precisa de ajuda para entender como fazer isso.

Para ajudar Carla a usar o ChatGPT para resumir textos longos, marque a alternativa correta:
<table style="text-align: center; width: 100%;"> 
<tr>
    <td style="text-align: left;">
    <img src="imgs/Prova_questoes/reusmo_texto_longo_resp.png" alt="reposta estrategia" width="50%"/>
    </td>
</tr>
</table>

## 3. Trabalhando com textos longos: system e message
No tópico anterior visualizamos como podemos ligar com texto muito longos, porém o que pode ser feito em casos que para além do texto a ser processado a instrução também é longa o que podemos fazer ?  

Para acessarmos a mensagem de input do `GPT` podemos acessar o [Playground][https://platform.openai.com/chat], através desse link podemos visualizar por exemplo que temos _"por trás do sistema"_
<table style="text-align: center; width: 100%;"> 
<tr>
    <td style="text-align: left;">
    <img src="imgs/playground.png" alt="reposta estrategia" width="50%"/>
    </td>
</tr>
</table>

É importante salientar que a utilização do Playground é paga
...
Essa plataforma é voltada para pessoas desenvolvedoras, cientistas de dados, empresas e profissionais de IA que desejam aproveitar os recursos avançados. Para acessá-la é necessário fazer o login na página [OpenAI platform](https://platform.openai.com/login).
...
__Ajustando as configurações__  
...
Para saber mais sobre a plataforma você pode acessar a página:
[OpenAI: Documentation.](https://developers.openai.com/api/docs/concepts)
...
Pense nas estratégias que você vai utilizar para fazer isso e faça o resumo, com o texto abaixo, obtido no [site Wikipédia](https://pt.wikipedia.org/wiki/Python). 
...
Para resumir textos longos uma estratégia interessante é dividir o textos em partes. Ao dividir o texto em seções menores, é possível analisar cada parte separadamente e resumir suas informações principais. Isso ajuda a lidar com o desafio de limitação de tamanho e também facilita o processo de compreensão e resumo.

A seguir temos um prompt com a primeira parte do texto, onde especificamos que o resumo deveria conter no máximo um parágrafo. Lembre-se que você pode especificar até mesmo a quantidade de caracteres que deseja.  
```text
...
Resumo:
```
Com esse prompt obtivemos o seguinte resultado:

```text
Python é uma linguagem de programação de alto nível, interpretada e orientada a objetos, lançada por Guido van Rossum em 1991. Com uma sintaxe clara e concisa, enfatiza a legibilidade do código e possui uma ampla biblioteca padrão e módulos de terceiros. É utilizada principalmente para processamento de textos, dados científicos e criação de CGIs para a web. Considerada uma das linguagens mais amadas pela comunidade de desenvolvedores, Python tem seu nome inspirado no grupo humorístico Monty Python.
```
Muito bom! Temos aí o resumo da primeira parte do texto. Após dividir o texto em partes, é importante se concentrar em cada parte separadamente.

Você pode usar diferentes chats para interagir com o modelo sobre cada seção. Isso permitirá que você se concentre em uma seção de cada vez e obtenha resumos mais específicos para cada parte.

Ao finalizar os resumos de cada parte, você pode integrá-los para criar um resumo geral do texto. Não se esqueça de revisar o conteúdo para garantir que o resumo geral seja coeso, mantenha a lógica do texto original e forneça uma visão abrangente dos principais pontos abordados.  

## 6. O que aprendemos?
Nessa aula, você aprendeu a:
- Delimitar um texto com muitos caracteres em partes menores e mais gerenciáveis;
- Desenvolver e aplicar estratégias para resumir textos com muitos caracteres;
- Conhecer e utilizar a OpenAI Playground para separar as instruções em system e a parte textual em message.
...
---
__Titulo:__ Estratégias para textos longos
